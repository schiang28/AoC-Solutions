import os
passport = '''ecl:hzl byr:1926 iyr:2010
...
for i in p:
    ...
    if len(x) == 8:
        ...
        if not any("cid" in z for z in x):
            ls.append(x)
...
for j in ls:
    ...
    for b in j:
        ...
        valid = False
        if first == "byr":
            if int(sec) >= 1920 and int(sec) <= 2002:
                valid = True
            else:
                valid = False
        if first == "iyr":
            if int(sec) >= 2010 and int(sec) <= 2020:
                valid = True
            else:
                valid = False
        if first == "eyr":
            if int(sec) >= 2020 and int(sec) <= 2030:
                valid = True
            else:
                valid = False
        if first == "hgt":
            if sec[-2:] == "cm":
                if int(sec[:-2]) >= 150 and int(sec[:-2]) <= 193:
                    valid = True
            elif sec[-2:] == "in":
                if int(sec[:-2]) >= 59 and int(sec[:-2]) <= 76:
                    valid = True
                else:
                    valid = False
            else:
        ...
        if first == "hcl":
            if any([c not in letter for c in sec[1:]]) or sec[0] != "#" or len(sec) != 7:
                valid = False
            else:
                valid = True

        if first == "ecl":
            if sec == "amb" or sec == "blu" or sec == "brn" or sec == "gry" or sec == "grn" or sec == "hzl" or sec == "oth":
                valid = True
            else:
                valid = False
